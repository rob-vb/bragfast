...
export async function EndpointBlock({ endpoint }: { endpoint: ApiEndpoint }) {
  return (
    <div
      id={endpoint.anchor}
      className="scroll-mt-14 md:scroll-mt-8 grid grid-cols-1 xl:grid-cols-[1fr_380px] gap-6 xl:gap-8 py-8 sm:py-10 border-b border-brand/10 last:border-b-0"
    >
      {/* Left: Prose */}
      <div className="min-w-0">
        <div className="flex items-center gap-2 sm:gap-3 mb-3 sm:mb-4 flex-wrap">
          <MethodBadge method={endpoint.method} />
          <code className="text-sm font-mono font-medium text-brand break-all">
            {endpoint.path}
          </code>
        </div>

        <h3 className="text-base sm:text-lg font-semibold text-brand mb-2">
          {endpoint.title}
        </h3>

        <p className="text-sm text-brand/70 leading-relaxed mb-5 sm:mb-6">
          {endpoint.description}
        </p>

        {endpoint.params && endpoint.params.length > 0 && (
          <div className="mb-5 sm:mb-6">
            <h4 className="text-xs font-semibold uppercase tracking-wider text-brand/50 mb-3">
              Parameters
            </h4>
      ...
      <div className="min-w-0 space-y-4 -mx-4 px-4 sm:mx-0 sm:px-0">
        ...
        <div>
          <h4 className="text-xs font-semibold uppercase tracking-wider text-brand/50 mb-2">
            Response{" "}
            <span className="text-brand/30 normal-case font-normal">

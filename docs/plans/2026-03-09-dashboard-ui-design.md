...
```
/dashboard              → Kitchen (home)
/dashboard/brands       → Brand list
/dashboard/brands/new   → Create brand
/dashboard/brands/[id]  → Edit brand
/dashboard/history      → Release history
/dashboard/keys         → API key management
```

All under `(dashboard)` route group with shared layout. Layout checks Better Auth session — no session redirects to `/login`.

## Pages

### Kitchen (Home) — `/dashboard`
...
**Create/Edit** pages (`/dashboard/brands/new`, `/dashboard/brands/[id]`):
...
Dashboard layout wraps all pages. Checks Better Auth session server-side. No session → redirect to `/login`. User data (ID, name) available to all child pages.

## Data Flow

All data fetched from existing `/api/v1/*` endpoints (brands, releases, api-keys). Dashboard is a consumer of the existing API — no new backend routes needed. Convex queries used directly where appropriate (e.g. userProfiles for credit balance).

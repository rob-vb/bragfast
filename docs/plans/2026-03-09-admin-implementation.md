...
**Goal:** Build a pixel-arcade-styled authenticated admin with Kitchen (stats + recent releases), Brands (CRUD), History (release list), and API Keys (manage) pages.

**Architecture:** Next.js App Router `(admin)` route group with shared layout. Layout authenticates via `convexBetterAuthNextJs` server-side, redirects to `/login` if no session. Pages use `fetchQuery`/`fetchMutation` from `convex/nextjs` for data. Pixel UI: Press Start 2P headings, sharp corners, offset box-shadows, `#F8AF3C`/`#4A3326`/`#FFF8F0` palette.
...
git commit -m "feat: add listByUser and getStats Convex queries for admin"
```
...
Create the `(admin)` route group with shared layout: top nav bar, auth check, pixel styling.
...
**Step 5: Verify** — run `npm run dev`, visit `/admin`. Should redirect to `/login` if not authenticated. If authenticated, should show top bar with nav tabs and "Kitchen" heading.

**Step 6: Commit**

```bash
git add src/lib/auth/get-session-user.ts src/app/\(admin\)/layout.tsx src/app/\(admin\)/page.tsx src/components/admin/nav.tsx
git commit -m "feat: add admin layout with pixel arcade nav bar and auth guard"
```
...
Create reusable styled components for the admin's pixel aesthetic. These are used by all four pages.

**Files:**
- Create: `src/components/admin/pixel-card.tsx`
- Create: `src/components/admin/pixel-button.tsx`
- Create: `src/components/admin/pixel-badge.tsx`
- Create: `src/components/admin/pixel-table.tsx`
...
```tsx
// src/components/admin/pixel-button.tsx
...
```tsx
// src/components/admin/pixel-table.tsx
...
### Task 4: Kitchen Page (Admin Home)
...
**Step 2: Verify** — visit `/admin` while logged in. Should see 4 stat cards and recent releases table.
    ...
    <div className="space-y-6">
      ...
      {brands.length === 0 ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {brands.map((brand) => (
            <Link key={brand._id} href={`/admin/brands/${brand.externalId}`}>
...
**Step 5: Fix brands API route auth** — the existing `/api/v1/brands` route uses API key auth (Bearer token). The admin uses session auth. Need to update the brands route to also accept session auth, OR create admin-specific API routes.
...
**Step 6: Verify** — visit `/admin/brands`, create a brand, edit it.

**Step 7: Commit**

```bash
git add src/app/\(admin\)/brands/ src/components/admin/brand-form.tsx
...
```tsx
// src/components/admin/history-filter.tsx
"use client";
...
**Step 3: Verify** — visit `/admin/history`, check filter buttons work.

**Step 4: Commit**

```bash
git add src/app/\(admin\)/history/ src/components/admin/history-filter.tsx
...
- Create: `src/components/admin/key-manager.tsx`
...
```tsx
// src/components/admin/key-manager.tsx
"use client";
...
**Step 4: Verify** — visit `/admin/keys`, generate a key, copy it, revoke it.

**Step 5: Commit**

```bash
git add src/app/\(admin\)/keys/ src/components/admin/key-manager.tsx
...
The brands API route (`/api/v1/brands`) may only support API key auth. The admin needs session auth to work with the same endpoints. Check and fix.
...
**Step 3: Verify** — test all admin pages end-to-end with session auth.

**Step 4: Commit**

```bash
git add src/app/api/
git commit -m "feat: add session auth support to API routes for admin"
```
...
**Files:**
- Modify: various admin components
...
```bash
git add .
git commit -m "fix: responsive polish for admin"
```

...
brag.fast is a CLI-first creation tool for developers. A developer runs `npx bragfast` from their terminal, authors and fills branded post templates in a locally-served browser Workspace, renders the output on their own machine, and copies it or schedules it through Buffer/Postiz. The hosted backend is thin: it stores the user's assets (templates, brands, drafts), handles auth and billing, and proxies scheduling.
...
**Target features:**
- `npx bragfast` CLI: browser device-flow login, serves localhost Workspace, reads local files + repo context, renders locally. The installed package also exposes a `brag` bin for global/local usage.
...
- **Tech stack**: Node/TypeScript CLI (`npx bragfast`) — reuses existing TS render core. No compiled-binary path in MVP.
